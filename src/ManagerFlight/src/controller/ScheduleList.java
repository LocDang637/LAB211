...
public class ScheduleList implements Serializable {
    ...
    public void addSchedule(ArrayList<Flight> list,ArrayList<Staff> listStaff) {
        String code;
        boolean isExist;
        ...
        for (int i = 0; i < listSchedule.size(); i++) {
            if (listSchedule.get(i).getForFlight().getNumber().equalsIgnoreCase(code)) {
                ...
                listSchedule.get(i).input(listSchedule,listStaff,newFlight);
                isExist = true;
            }
        }
        ...
        if (!isExist) {
            ...
            newSchedule.input(listSchedule,listStaff,newFlight);
